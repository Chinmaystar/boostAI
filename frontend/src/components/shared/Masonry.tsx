...
const Masonry: React.FC<MasonryProps> = ({
  ...
  // We need to set the container height to the max column height
  useEffect(() => {
    if (containerRef.current && grid.length > 0) {
      const maxColHeight = Math.max(...grid.map(i => i.y + i.h));
    ...
    <div ref={containerRef} className="list">
      {grid.map(item => {
        return (
          ...
          >
            {item.content ? (
              <div className="w-full h-full relative">{item.content}</div>
            ) : (
